// helpers/contexts/NetworkContext.tsx
import React, {
  createContext,
  useContext,
  useEffect,
...
export const NetworkProvider = ({
  ...
  const checkApiReachability = useCallback(async (): Promise<boolean> => {
    try {
      ...
      setStatus((prev) => ({
        ...
        reconnectAttempts: isReachable ? 0 : prev.reconnectAttempts,
      }));
  ...
  useEffect(() => {
    ...
    // Vérification périodique du Socket
...
export const useNetwork = () => {
  ...
  if (!context) {
    if (__DEV__) {
      console.warn("⚠️ useNetwork must be used within NetworkProvider");
    }
